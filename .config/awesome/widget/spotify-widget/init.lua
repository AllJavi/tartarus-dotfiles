...
local function worker(user_args)
    ...
    -- 9local font = args.font or 'Play 9'
    local font = 'JetBrains Mono Medium Nerd Font Complete 10'
        ...
        -- },
	{
	    ...
	    if artist_to_display ~= '' then
            ...
            if self:get_children_by_id('titlew')[1]:get_markup() ~= title_to_display then
                self:get_children_by_id('titlew')[1]:set_markup('<span color=\'#282828\'>' .. title_to_display .. connector .. artist_to_display .. '</span>')
            end
        end
    }
    ...
    local update_widget_text = function(widget, stdout, _, _, _)
        if string.find(stdout, 'Error: Spotify is not running.') ~= nil then
            widget:set_text('','Waiting for Spotify ...')
